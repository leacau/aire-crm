...
import { mapClient } from '@/lib/server/clients';
import { serializeDocument } from '@/lib/server/firestore';
import { mapPaymentEntry } from '@/lib/server/payments';
...
export class AdvisorReportApiError extends Error {
  constructor(
    message: string,
...
export async function buildAdvisorReportsServer(rawBody: unknown, requester: ServerUser): Promise<AdvisorReportData[]> {
  ...
  if (!hasServerManagementPrivileges(requester) && (advisorIds.length > 1 || advisorIds[0] !== requester.uid)) {
    throw new AdvisorReportApiError('Forbidden', 403);
  }
  ...
  return advisorIds.flatMap((advisorId) => {
