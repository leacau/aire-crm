...
const isRecoverableOpportunityApiWriteError = (error: unknown): boolean => {
    const maybeError = error as { status?: number; code?: string; message?: string };
    return Boolean(
        (typeof maybeError.status === 'number' && maybeError.status >= 500) ||
        maybeError.code === 'FIRESTORE_UNAVAILABLE' ||
        maybeError.code === 'FIRESTORE_INDEX_REQUIRED' ||
        maybeError.code === 'INTERNAL_ERROR' ||
        maybeError.message?.includes('Firestore no pudo completar')
    );
};

const updateOpportunityDirectly = async (
    id: string, 
    ...
    if ('manualUpdateHistory' in updateData) delete updateData.manualUpdateHistory;

    const stageChanged = data.stage && data.stage !== originalData.stage;
    if (stageChanged) updateData.stageChangedAt = serverTimestamp();
    ...
    if (data.stage === 'Cerrado - Ganado' && originalData.stage !== 'Cerrado - Ganado') {
        await createCommercialItemsFromOpportunity({ ...originalData, ...data, id }, userId, userName);
    }
    ...
    if (data.agencyId === '' || data.agencyId === undefined) updateData.agencyId = deleteField();
    ...
    if (isRenewal || ('finalizationDate' in data && !data.finalizationDate)) cacheData.finalizationDate = undefined;
    ...
    if (pendingInvoices && pendingInvoices.length > 0) {
        for (const invoiceData of pendingInvoices) {
            await createInvoice({ ...invoiceData, opportunityId: id }, userId, userName, ownerName);
        }
    }
        ...
        try {
            ...
            await autoUpdateCoachingSession(
                ...
                isClosingWonProposal
                    ? { createIfMissing: false, completeIfActive: true, updateExistingIfMissing: true }
                    : undefined
            );
        } catch (e) {
            console.error('Error auto-updating coaching:', e);
...
export const updateOpportunity = async (
    ...
    try {
        ...
        if (pendingInvoices && pendingInvoices.length > 0) invalidateCache('invoices');
        return persisted;
    } catch (error) {
        if (!isRecoverableOpportunityApiWriteError(error)) throw error;
        console.warn('Falling back to Firestore opportunity update after API write failed:', error);
        return updateOpportunityDirectly(id, data, userId, userName, ownerName, pendingInvoices, options);
    }
};
