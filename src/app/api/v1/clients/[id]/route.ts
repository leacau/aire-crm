...
import { getClientForOrganization, updateClientOnServer, updateClientSchema } from '@/modules/clients/server';
...
export async function GET(request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    return NextResponse.json({ data: await getClientForOrganization(id, user.organizationId) });
  } catch (error) {
...
export async function PATCH(request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    const { id } = await params;
    await updateClientOnServer(id, updateClientSchema.parse(json), user);
